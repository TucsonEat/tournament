...
function TucsonEatsTournament() {
  ...
  const handleVote = async () => {
    ...
    try {
      ...
      // Queue restaurant digest notification
      queueVoteNotification(selectedId, votedRestaurant?.name, reg.firstName);
...
function TucsonEatsAdmin() {
  ...
  const [customerVotesModal, setCustomerVotesModal] = useState(null); // email to show votes for
  ...
  // ── Restaurant vote digest notifications ──
  const saveNotifySettings = async (newSettings) => {
    setNotifySettingsSaving(true);
    await storage.set(NOTIFY_SETTINGS_KEY, JSON.stringify(newSettings));
  ...
  const queueVoteNotification = async (restaurantId, restaurantName, voterName) => {
    try {
      let pending = [];
      try { const r = await storage.get(NOTIFY_PENDING_KEY); if (r) pending = JSON.parse(r.value); } catch {}
      const existing = pending.find(p => p.restaurantId === restaurantId);
      if (existing) {
        existing.voteCount = (existing.voteCount || 0) + 1;
        existing.voters = [...(existing.voters || []), voterName].slice(-10);
      } else {
        pending.push({ restaurantId, restaurantName, voteCount: 1, voters: [voterName], firstVoteAt: Date.now() });
      }
      await storage.set(NOTIFY_PENDING_KEY, JSON.stringify(pending));
    } catch {}
  };

  const sendDailyDigest = async (force = false) => {
    try {
      ...
      try {
        const lastSent = await storage.get(NOTIFY_LAST_SENT_KEY);
        if (lastSent && lastSent.value === today && !force) return;
      } catch {}
      // Check if it's past the send time
      if (!force) {
        const [h, m] = settings.sendTime.split(":").map(Number);
        const now = new Date();
      ...
      // Send email for each restaurant with emailNotify enabled
      const restsToNotify = restaurants.filter(r => r.emailNotify && r.email);
      for (const rest of restsToNotify) {
        const pItem = pending.find(p => p.restaurantId === rest.id);
        if (!pItem && !force) continue;
        const voteCount = pItem?.voteCount || 0;
        const voters = pItem?.voters || [];
        try {
          await fetch(WORKER_URL, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              type: "restaurant_digest",
              restaurantEmail: rest.email,
              restaurantName: rest.name,
              voteCount,
              voters,
              sendDate: new Date().toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" }),
            }),
          });
        } catch {}
      }
      // Clear pending and mark sent
      await storage.set(NOTIFY_PENDING_KEY, JSON.stringify([]));
      await storage.set(NOTIFY_LAST_SENT_KEY, today);
      if (force) showToast(`✅ Digest sent to ${restsToNotify.length} restaurant(s)`);
    } catch {}
  };

  // Check digest on load and every minute
  useEffect(() => {
    if (!authed) return;
    sendDailyDigest();
    const iv = setInterval(() => sendDailyDigest(), 60000);
    return () => clearInterval(iv);
  }, [authed, notifySettings, restaurants]);
  ...
  const handleDeleteCustomer = async () => {
    if (!custEditModal || custEditModal === "new") return;
    const idx = registry.findIndex(r => r.email === custEditModal.email && r.votedAt === custEditModal.votedAt);
    if (idx === -1) return;
    // Decrement vote count for the restaurant they voted for
    if (custEditModal.votedFor) {
      const updatedRests = restaurants.map(r =>
        r.id === custEditModal.votedFor ? { ...r, votes: Math.max(0, r.votes - 1) } : r
      );
      await storage.set(STORAGE_KEY, JSON.stringify(updatedRests));
      setRests(updatedRests);
    }
    const updated = registry.filter((_, i) => i !== idx);
    await storage.set(REGISTRY_KEY, JSON.stringify(updated));
    setRegistry(updated);
    setCustEditModal(null);
    setConfirm(null);
  ...
  return (
    <>
                          ...
                          <tbody>
                            {filteredCustomers.map((r, i) => {
                              ...
                              const timeOnSite = r.sessionStart && r.votedAt
                                ...
                                : "—";
                              return (
                                <tr key={i} onClick={() => setCustomerVotesModal(r.email)} style={{ cursor: "pointer" }} title="Click to see votes">
                                  ...
                                  <td>
                                    <button
                                      onClick={(e) => { e.stopPropagation(); toggleEmailNotification(r); }}
                                  ...
                                  <td>
                                    <button
                                      onClick={(e) => { e.stopPropagation(); toggleAllowRevote(r); }}
            ...
            {tab === "settings" && (
              <div>
                ...
                <div className="adm-table-wrap" style={{ marginTop: "24px" }}>
                  <div className="adm-table-header">
                    <div>
                      <h3>🔔 Restaurant Vote Digest</h3>
                      <p style={{ margin: "4px 0 0", fontSize: "12px", color: "var(--text-dim)" }}>Send a daily email to restaurants summarizing new votes received</p>
                    </div>
                  </div>
                  <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: "20px" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", background: "rgba(255,255,255,.03)", borderRadius: "10px", border: "1px solid var(--border)" }}>
                      <div>
                        <div style={{ fontWeight: 700, fontSize: "14px" }}>Enable Daily Digest</div>
                        <div style={{ fontSize: "12px", color: "var(--text-dim)", marginTop: "3px" }}>Restaurants with notifications ON will receive a daily vote summary</div>
                      </div>
                      <button
                        onClick={() => saveNotifySettings({ ...notifySettings, enabled: !notifySettings.enabled })}
                        style={{
                          display: "inline-flex", alignItems: "center", gap: "8px",
                          padding: "8px 18px", borderRadius: "20px", fontSize: "13px",
                          fontWeight: 700, cursor: "pointer", fontFamily: "inherit",
                          border: "1.5px solid",
                          background: notifySettings.enabled ? "rgba(75,134,62,.2)" : "rgba(255,255,255,.04)",
                          borderColor: notifySettings.enabled ? "var(--green)" : "var(--border)",
                          color: notifySettings.enabled ? "var(--green-l)" : "var(--text-dim)",
                          transition: "all .2s", minWidth: "100px", justifyContent: "center",
                        }}
                      >
                        {notifySettings.enabled ? "🟢 Enabled" : "⚫ Disabled"}
                      </button>
                    </div>

                    {/* Send time picker */}
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", background: "rgba(255,255,255,.03)", borderRadius: "10px", border: "1px solid var(--border)", opacity: notifySettings.enabled ? 1 : 0.5 }}>
                      <div>
                        <div style={{ fontWeight: 700, fontSize: "14px" }}>Daily Send Time</div>
                        <div style={{ fontSize: "12px", color: "var(--text-dim)", marginTop: "3px" }}>Digest emails will be sent once per day at this time</div>
                      </div>
                      <input
                        type="time"
                        value={notifySettings.sendTime}
                        disabled={!notifySettings.enabled}
                        onChange={e => setNotifySettings(s => ({ ...s, sendTime: e.target.value }))}
                        onBlur={() => saveNotifySettings(notifySettings)}
                        className="adm-input"
                    ...
                    {/* How it works */}
                    <div style={{ background: "rgba(75,134,62,.06)", border: "1px solid rgba(75,134,62,.15)", borderRadius: "10px", padding: "14px 18px" }}>
                      <div style={{ fontWeight: 700, fontSize: "12px", color: "var(--green-l)", marginBottom: "8px", letterSpacing: "1px", textTransform: "uppercase" }}>How it works</div>
                      <ul style={{ margin: 0, padding: "0 0 0 16px", fontSize: "12px", color: "var(--text-dim)", lineHeight: 2 }}>
                        <li>Each vote is queued for the restaurant's daily digest</li>
                        <li>At the scheduled time, one email is sent per restaurant</li>
                        <li>Only restaurants with <strong style={{ color: "var(--text-muted)" }}>🔔 Notifications ON</strong> and an email address receive digests</li>
                        <li>Enable/disable per restaurant in the <strong style={{ color: "var(--text-muted)" }}>Restaurants tab</strong></li>
                      </ul>
                    </div>
      ...
      {/* ── CUSTOMER VOTES MODAL ── */}
      {customerVotesModal && (() => {
        const customerVotes = registry.filter(r => r.email?.toLowerCase() === customerVotesModal.toLowerCase());
        const customer = customerVotes[0];
        return (
          <div className="adm-modal-bg" onClick={e => e.target === e.currentTarget && setCustomerVotesModal(null)}>
            <div className="adm-modal" style={{ maxWidth: "520px" }}>
              <h3>🗳️ Vote History</h3>
              <div style={{ marginBottom: "16px", padding: "12px 16px", background: "rgba(255,255,255,0.04)", borderRadius: "10px" }}>
                <div style={{ fontWeight: 700, fontSize: "16px" }}>{customer?.firstName || "—"}</div>
                <div style={{ color: "var(--text-muted)", fontSize: "13px" }}>{customer?.email}</div>
                {customer?.phone && <div style={{ color: "var(--text-dim)", fontSize: "12px" }}>{customer.phone}</div>}
              </div>
              <div style={{ marginBottom: "12px", color: "var(--text-dim)", fontSize: "12px", fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase" }}>
                {customerVotes.length} Vote{customerVotes.length !== 1 ? "s" : ""}
              </div>
              {customerVotes.length === 0 ? (
                <div style={{ textAlign: "center", color: "var(--text-dim)", padding: "24px" }}>No votes found</div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: "10px", maxHeight: "320px", overflowY: "auto" }}>
                  {customerVotes.map((v, i) => {
                    const rest = restaurants.find(x => x.id === v.votedFor);
                    const voteDate = v.votedAt ? new Date(v.votedAt) : null;
                    return (
                      <div key={i} style={{ display: "flex", alignItems: "center", gap: "14px", padding: "12px 16px", background: "var(--card)", borderRadius: "10px", border: "1px solid var(--border)" }}>
                        <div style={{ fontSize: "24px" }}>{rest ? (CUISINE_EMOJI[rest.cuisine] || "🍽️") : "🍽️"}</div>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 700, fontSize: "15px" }}>{rest?.name || <span style={{ color: "var(--text-dim)" }}>Unknown Restaurant</span>}</div>
                          {rest?.cuisine && <div style={{ color: "var(--text-muted)", fontSize: "12px" }}>{rest.cuisine}</div>}
                        </div>
                        <div style={{ textAlign: "right", fontSize: "12px", color: "var(--text-dim)" }}>
                          {voteDate ? (
                            <>
                              <div style={{ fontWeight: 700, color: "var(--text-muted)" }}>{voteDate.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</div>
                              <div>{voteDate.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}</div>
                            </>
                          ) : "—"}
                        </div>
                      </div>
              ...
              <div style={{ marginTop: "20px", display: "flex", justifyContent: "flex-end" }}>
                <button className="adm-btn adm-btn-outline" onClick={() => setCustomerVotesModal(null)}>Close</button>
              </div>
            </div>
          </div>
        );
            ...
            <div style={{ display: "flex", gap: "8px", marginTop: "20px" }}>
              ...
              {custEditModal !== "new" && (
                <button
                  ...
                  onClick={() => setConfirm({
                    msg: `Delete ${custEditModal.firstName || custEditModal.email}? This will also remove their vote and cannot be undone.`,
